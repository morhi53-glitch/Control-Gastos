...
// ---- Utility helpers ----
// === Sync & Queue config ===
const API_BASE = ""; // TODO: configura tu backend (https://tu-dominio.com)
const QUEUE_KEY = "boat-expenses-queue";
function loadQueue() { try { return JSON.parse(localStorage.getItem(QUEUE_KEY)||"[]"); } catch { return []; } }
function saveQueue(q) { try { localStorage.setItem(QUEUE_KEY, JSON.stringify(q)); } catch {} }
async function sendToServer(exp){
  if(!API_BASE){ return false; }
  try{ const res = await fetch(`${API_BASE}/api/expenses`,{method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(exp)}); return res.ok; }catch{ return false; }
}
async function syncQueue(setPending){
  let q = loadQueue();
  if(q.length===0){ setPending?.(0); return {sent:0,left:0}; }
  const remaining = []; let sent = 0;
  for(const item of q){ const ok = await sendToServer(item); if(ok) sent++; else remaining.push(item); }
  saveQueue(remaining); setPending?.(remaining.length); return {sent,left:remaining.length};
}
function enqueueExpense(exp){ const q = loadQueue(); q.push(exp); saveQueue(q); }
const CATEGORIES = [
  "Combustible",
  "Mantenimiento",
  "Amarras/Puerto",
...
export default function BoatExpensesApp() {
  ...
  useEffect(() => {
    ...
    if (navigator.serviceWorker) {
      navigator.serviceWorker.addEventListener('message', (evt) => {
        if (evt?.data?.type === 'SYNC_EXPENSES') {
          syncQueue(setPending);
        }
      });
      navigator.serviceWorker.ready.then((reg) => {
        if (reg.sync) { reg.sync.register('sync-expenses').catch(()=>{}); }
      });
    }
    return () => {
  ...
  function addExpense(exp) {
    const record = { id: uid(), ...exp };
    setExpenses((prev) => [record, ...prev]);
    enqueueExpense(record);
    setPending(loadQueue().length);
    if (isOnline) { syncQueue(setPending); }
  }, ...prev]);
  }
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={()=> syncQueue(setPending)} title="Sincronizar con servidor">
              Sincronizar ({pending})
            </Button>
...
// SW minimalista: cache-first para estáticos y network-first para documentos + Background Sync (señal a la app)
  ...
  if (event.tag === 'sync-expenses') {
    event.waitUntil(
      self.clients.matchAll({ includeUncontrolled: true, type: 'window' }).then((clients) => {
        clients.forEach((c) => c.postMessage({ type: 'SYNC_EXPENSES' }));
      })
    );
  }
});
